...
export function InventoryDetail({ unit }: { unit: InventoryUnit }) {
  ...
  useEffect(() => {
    document.title = `${unit.title} | Turbo RV`;
    return () => {
      document.title = 'Turbo RV';
    };
  }, [unit.title]);
        ...
        <div className="flex min-w-0 flex-col gap-5">
          ...
          <div className="relative overflow-visible rounded-2xl border border-neutral-200 bg-white p-4 shadow-[0_2px_10px_rgba(0,0,0,0.06)] sm:p-5">
            {!isAvailable || users.length === 0 ? (
              <>
                <span className="inline-flex rounded-md bg-neutral-100 px-2.5 py-1 text-xs font-bold tracking-wide text-neutral-700 uppercase">
                  No specialists available
                </span>
                <div className="mt-3">
                  <h2 className="text-2xl leading-tight font-bold text-neutral-900">Specialists are not available</h2>
                  <p className="mt-1 text-sm text-neutral-500">
                    {unit.isTooLowToShow
                      ? 'No specialists are available. Leave a message and we can still help you with the best price for this unit.'
                      : 'No specialists are available right now. Leave a message and we will get you the best price when we are back.'}
                  </p>
                </div>

                <button
                  type="button"
                  className="mt-4 flex h-12 w-full cursor-pointer items-center justify-center gap-2 rounded-xl bg-neutral-900 px-4 py-3 text-lg font-bold text-white transition hover:bg-neutral-900/80"
                  onClick={() => setContactOpen(true)}
                >
                  Leave a message
                  <MessageCircle className="size-4 shrink-0 text-white sm:size-5" strokeWidth={2} />
                </button>
              </>
            ) : unit.isTooLowToShow ? (
              <>
            ...
            {isAvailable && users.length > 0 ? (
              <div
                className={`mt-4 flex flex-wrap items-center justify-center gap-5 text-lg ${
                  isAvailable && users.length > 0 ? 'text-neutral-700' : 'text-neutral-500'
                }`}
              >
